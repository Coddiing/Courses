...
class UsersController @Inject()(protected val dbConfigProvider: DatabaseConfigProvider, cc: MessagesControllerComponents,
  ...
  def secretary = Action { implicit request =>
    ...
    val obj = StudentsData // Assign StudentsData singleton to obj
    val bew = BewerberListe // Assign BewerberListe singleton to bew

    // Sir, these variables are passed as the attributes of the HTML elements in the secretary page
    // Just like it is in the '.jsp' source.
    val isChecked = obj.getCheckedbox(1, obj.getIsChecked )
    val isDisabled = bew.isInvalidate(bew.getIsAdmin && !bew.getIsAAA)
    val getDateZulSent = obj.ignoreNull( obj.getDate_Zul_Sent )
    val isReadOnly = bew.isReadOnly( bew.getIsAAA )
    Ok(views.html.secretary(
                              userName, lastLoggedIn, obj.getBemlang,
                              obj.getVorname, obj.getNachname, obj.getStaat,
                              obj.getZulassung, obj.getSitzungNr, isChecked,
                              obj.getBemerkung, getDateZulSent, isReadOnly,
                              isDisabled
                            ))
  }
